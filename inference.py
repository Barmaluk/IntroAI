...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        if noise == None:
            positions[self.getJailPosition()] = 1.0

        position = gameState.getPacmanPosition()

        for pos in self.allPositions:
            model = self.getObservationProb(noise, position, pos, self.getJailPosition())
            positions[pos] = self.beliefs[pos] * model
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        predictor = util.Counter()

        for position in self.allPositions:
            newDistance = self.getPositionDistribution(gameState, position)
            for newPosition in newDistance:
                predictor[newPosition] = predictor[newPosition] + self.beliefs[position] * newDistance[newPosition]
        self.beliefs = predictor
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        if observation == None:
            ...
            for location, value in beliefDistribution.items():
                model = self.getObservationProb(observation, gameState.getPacmanPosition(), location, self.getJailPosition())
                beliefDistribution[location] = model * value 
            weight = DiscreteDistribution(beliefDistribution)
            
            if weight.total() == 0:
                self.initializeUniformly(gameState)
            else:
                beliefs = DiscreteDistribution(beliefDistribution)
                for i in range(self.numParticles):
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
            newDistance = self.getPositionDistribution(gameState, particle)
            newPosition = DiscreteDistribution(newDistance)
            newParticles.append(newPosition.sample())
