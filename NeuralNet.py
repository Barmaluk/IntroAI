...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):
        ...
        """YOUR CODE"""
        act = [1.0] + inActs
        weightedSum = self.getWeightedSum(act)
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for index in range(len(self.weights)):
            ...
            change = inActs[index] * (alpha * delta) 
            newWeight = weight + change

            myWeights.append(newWeight)
            weight = newWeight
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        for layer in range(self.numLayers):
            currentLayer = []
            for perceptron in self.layers[layer]:
                value = perceptron.sigmoidActivation(prev)
                currentLayer.append(value)
            output.append(currentLayer)
            prev = currentLayer
        ...
        for example in examples:#for each example
            ...
            for layerNum in range(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in range(len(layer)):
                    ...
                    error = 0.0
                    for hidden in range(len(nextLayer)):
                        delta = nextLayer[hidden].weights[neuronNum + 1] * deltas[0][hidden]
                        error = error + delta
                    delta = gPrime * error
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxsize, startNNet = None):
    ...
    for input, output in examplesTest:
        myActivation = nnet.feedForward(input)
        endActivation = myActivation[-1]

        error = False
        for i in range(len(endActivation)):
            if (round(endActivation[i])) != output[i]:
                error = True
                testError += 1
        ...
        testAccuracy = testCorrect / tests
