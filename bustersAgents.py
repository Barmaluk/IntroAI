...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        bestAction = None
        closestDistanceB = float('inf')

        for action in legal:
            successor = Actions.getSuccessor(pacmanPosition, action)
            distance = self.distancer.getDistance(closestPosition, successor)
        ...
        for ghostDistribution in livingGhostPositionDistributions:
            ...
            for position in ghostDistribution:
                if ghostDistribution[position] > prob_best:
                    best = position
                    prob_best = ghostDistribution[position]
        ...
        closestAction = float('inf')
        bestAction = None

        for action in legal:
            successor = Actions.getSuccessor(pacmanPosition, action)
            distance = self.distancer.getDistance(closestGhostPosition, successor)
            if distance < closestAction:
                closestAction = distance
                bestAction = action
        return bestAction
        '''